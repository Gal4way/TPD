...
class PLMSSampler(object):
    ...
    def p_sample_plms(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        x_new=torch.cat([x,kwargs['inpaint_image'],kwargs['inpaint_mask'],kwargs['posemap'],kwargs['densepose']],dim=1)
        e_t = get_model_output(x_new, t)
        if len(old_eps) == 0:
            # Pseudo Improved Euler (2nd order)
            x_prev, pred_x0 = get_x_prev_and_pred_x0(e_t, index)
            x_prev_new=torch.cat([x_prev,kwargs['inpaint_image'],kwargs['inpaint_mask'],kwargs['posemap'],kwargs['densepose']],dim=1)
